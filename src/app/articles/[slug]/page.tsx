  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <nav className="flex items-center gap-2 text-xs mb-4" style={{ color: "var(--text-secondary)" }}>
        ...
        <span>/</span>
        <Link href="/articles" className="hover:text-dk-red">مقالات</Link>
        <span>/</span>
        <span style={{ color: "var(--text)" }}>{article.title}</span>
      </nav>

      <article className="rounded-2xl border overflow-hidden" style={{ background: "var(--panel)", borderColor: "var(--border)" }}>
        ...
        <div className="p-6 md:p-8">
          <div className="flex items-center gap-3 text-[11px] mb-4" style={{ color: "var(--text-muted)" }}>
            <span className="font-bold px-2 py-1 rounded-lg" style={{ background: "var(--bg)", color: "#ef4050" }}>
              {article.category}
            </span>
            <span>{article.date}</span>
            <span>⏱ {article.readTime}</span>
          </div>
          <h1 className="text-xl font-extrabold leading-8 mb-6">{article.title}</h1>
          <div className="space-y-4 text-sm leading-8" style={{ color: "var(--text-secondary)" }}>
            {body.map((p, i) => (
              <p key={i}>{p}</p>
            ))}
          </div>
      ...
      <div className="mt-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {ARTICLES.filter((a) => a.id !== slug).slice(0, 3).map((a) => (
            <Link
              key={a.id}
              href={`/articles/${a.id}`}
            ...
            >
              <div className="relative aspect-[16/9] overflow-hidden" style={{ background: "var(--bg)" }}>
                <Image src={a.image} alt={a.title} fill sizes="33vw" className="object-cover group-hover:scale-105 transition-transform duration-300" />
              </div>
              <div className="p-3">
                <h3 className="text-xs font-bold leading-5 line-clamp-2">{a.title}</h3>
              </div>
            </Link>
          ))}
        </div>
