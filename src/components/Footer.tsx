...
export default function Footer() {
  return (
    <footer className="border-t mt-10" style={{ background: "var(--panel)", borderColor: "var(--border)", color: "var(--text)" }}>
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div>
            <h3 className="text-sm font-bold mb-3">فروشگاه اینترنتی دیجی‌کلون</h3>
            <p className="text-xs leading-6" style={{ color: "var(--text-secondary)" }}>
              دیجی‌کلون، اولین و بزرگ‌ترین فروشگاه اینترنتی نمونه ایرانی، با ده‌ها هزار
              کالای دیجیتال، خدمات پس از فروش و ضمانت اصالت کالا.
            </p>
        ...
        <div className="mt-8 pt-6 border-t flex flex-col sm:flex-row items-center justify-between gap-3" style={{ borderColor: "var(--border)" }}>
          <p className="text-xs" style={{ color: "var(--text-secondary)" }}>
            © ۱۴۰۳ — فروشگاه دیجی‌کلون (پروژه نمونه‌کار)
          </p>
          <div className="flex items-center gap-4">
            <Link href="/about" className="text-xs hover:text-dk-red transition-colors" style={{ color: "var(--text-secondary)" }}>
              نماد اعتماد الکترونیکی
            </Link>
            <Link href="/terms" className="text-xs hover:text-dk-red transition-colors" style={{ color: "var(--text-secondary)" }}>
              شرایط استفاده
            </Link>
            <Link href="/privacy" className="text-xs hover:text-dk-red transition-colors" style={{ color: "var(--text-secondary)" }}>
              حریم خصوصی
            </Link>
          </div>
        </div>
